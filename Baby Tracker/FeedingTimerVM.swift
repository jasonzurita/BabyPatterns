...
class FeedingTimerVM : BaseVM {
    private var feedingsInProgress:[FeedingTimer] = []
    weak var feedingModel:FeedingVM?
    
    init(feedingModel:FeedingVM? = nil) {
        self.feedingModel = feedingModel
    }
    ...
    func makeAndAddFeeding(feeding:FeedingTimer) -> FeedingEvent? {
        ...
        case .nursing:
            ...
            feedingModel?.nursing.nursingEvents.append(event as! NursingEvent)
            return event
        case .bottle:
            break
        case .pumping:
    ...
    func feedingInProgress(type:FeedingType, side:FeedingSide) -> FeedingTimer? {
        ...
        guard feedings.count == 1, let feeding = feedings.first else {
            debugPrint(string: "Warning! More than one feeding of the same type and side...")
            return nil
        }
        return feeding
    }
